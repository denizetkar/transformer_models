...
def define_performance(num_parallel_calls=True, inter_op=True, intra_op=True,
                       ...
                       all_reduce_alg=True, per_process_gpu_memory_fraction=True):
    ...
    if per_process_gpu_memory_fraction:
        flags.DEFINE_float(name="per_process_gpu_memory_fraction", default=None, lower_bound=0, upper_bound=1,
                           help=help_wrap("Determines the fraction of GPU memory "
                                          "to be allocated for the tensorflow session."
                                          "See tf.GPUOptions for more information."))
