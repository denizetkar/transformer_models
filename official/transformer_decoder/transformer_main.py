...
def model_fn(features, mode, params):
    """Defines how to train, evaluate and predict from the transformer_decoder model."""
    with tf.variable_scope("model"):
        inputs = features

        # Create model and get output logits.
        model = transformer.TransformerDecoder(params, mode)
        ...
        if mode == tf.estimator.ModeKeys.PREDICT:
            ...
            return tf.estimator.EstimatorSpec(
                ...
                predictions=logits,
                export_outputs={
                    "generate": tf.estimator.export.PredictOutput(logits)
                })
        ...
        # it is known from TransformerDecoder that the first two dimensions of logits
        # are the dimensions of self-targets. Note that the ambiguous shape of logits
        # is not a problem when computing xentropy, because padded_cross_entropy_loss
        # resolves the shape on the TPU.
        logits.set_shape(inputs.shape.as_list() + logits.shape.as_list()[2:])

        # Calculate model loss.
        # xentropy contains the cross entropy loss of every nonpadding token in the
        # self-targets.
        xentropy, weights = metrics.padded_cross_entropy_loss(
            logits, inputs, params["label_smoothing"], params["vocab_size"])
...
def define_transformer_flags():
    ...
    # Add transformer_decoder specific flags
    flags.DEFINE_enum(
    ...
    flags_core.set_defaults(data_dir="/tmp/generate_en",
...
def run_transformer(flags_obj):
    """Create tf.Estimator to train and evaluate transformer_decoder model.
    ...
    # Train and evaluate transformer_decoder model
