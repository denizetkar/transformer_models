...
# ==============================================================================
"""Beam search to find the generated sequence with the highest probability.
...
class SequenceBeamSearch(object):
    ...
    def _create_initial_state(self, initial_ids, initial_cache):
        ...
        Args:
          initial_ids: initial ids to pass into the symbols_to_logits_fn.
            int tensor with shape [batch_size, initial_length]
        ...
        cur_index = tf.shape(initial_ids)[1] - 1

        # Create alive sequence with shape [batch_size, beam_size, initial_length+1]
        ...
        flat_ids = _flatten_beam_dim(alive_seq)  # [batch_size * beam_size, initial_length+1]
        flat_cache = nest.map_structure(_flatten_beam_dim, alive_cache)

        # Create flat logits with shape [batch_size*beam_size, initial_length, vocab_size]
        flat_ids, flat_cache = self.symbols_to_logits_fn(flat_ids, 0, flat_cache)

        # Clip the last unintended logit value.
        flat_ids = _flat_clip_last_length(flat_ids)  # [batch_size * beam_size, initial_length]
        flat_cache = nest.map_structure(_flat_clip_last_length, flat_cache)

        # Unflatten logits to shape [batch_size, beam_size, initial_length, vocab_size]
        alive_logits = _unflatten_beam_dim(flat_ids, self.batch_size, self.beam_size)

        # Create initial log probabilities with shape [batch_size, beam_size]
        alive_logits_withoutbeam = alive_logits[:, 0, :, :]
        initial_ids_withoutzeros = initial_ids[:, 1:]
        initial_log_probs = _get_initial_log_probs(_log_prob_from_logits(alive_logits_withoutbeam),
                                                   initial_ids_withoutzeros)
        initial_log_probs = _expand_to_beam_size(initial_log_probs, self.beam_size)
        ...
        alive_log_probs = tf.constant(
            [[0.] + [-float("inf")] * (self.beam_size - 1)])
        alive_log_probs = tf.tile(alive_log_probs, [self.batch_size, 1])

        # Add initial log probabilities.
        ...
        state_shape_invariants = {
            ...
            _StateKeys.ALIVE_SEQ: tf.TensorShape([None, self.beam_size, None]),  # [batch, beam, length]
            _StateKeys.ALIVE_LOG_PROBS: tf.TensorShape([None, self.beam_size]),  # [batch, beam]
            _StateKeys.ALIVE_CACHE: nest.map_structure(
                _get_shape_keep_last_dim, alive_cache),
            _StateKeys.FINISHED_SEQ: tf.TensorShape([None, self.beam_size, None]),  # [batch, beam, length]
            _StateKeys.FINISHED_SCORES: tf.TensorShape([None, self.beam_size]),  # [batch, beam]
            _StateKeys.FINISHED_FLAGS: tf.TensorShape([None, self.beam_size])  # [batch, beam]
        }
    ...
    def _grow_alive_seq(self, state):
        ...
        # Get logits for the next candidate IDs for the alive sequences. Get the new
        # cache values at the same time.
        flat_ids = _flatten_beam_dim(alive_seq)  # [batch_size * beam_size, length]
        flat_cache = nest.map_structure(_flatten_beam_dim, alive_cache)

        flat_last_ids = flat_ids[:, -1:]
        flat_logits, flat_cache = self.symbols_to_logits_fn(flat_last_ids, i, flat_cache)
        flat_logits = tf.squeeze(flat_logits, axis=[1])
...
def sequence_beam_search(
    ...
    Args:
      ...
      initial_ids: Starting ids for each batch item.
        int32 tensor with shape [batch_size, initial_length]
...
def _get_initial_log_probs(log_probs, initial_ids):
    """Get log probabilities of given initial sequence.

    Reduces log_probs from [batch_size, initial_length, vocab_size] into
    [batch_size, initial_length] by taking index values between [0, vocab_size)
    from initial_ids.

    Args:
      log_probs: Log probabilities of each token between [0, vocab_size)
        for data point between [0, batch_size) and the next sequence position
        between (0, initial_length].
        float32 tensor with shape [batch_size, initial_length, vocab_size]
      initial_ids: Index number of tokens for data point between [0, batch_size)
        and the next sequence position between (0, initial_length].
        int32 tensor with shape [batch_size, initial_length]

    Returns:
      Initial log probabilities for each data point between [0, batch_size)
        float32 tensor with shape [batch_size]
    """
    batch_size = tf.shape(log_probs)[0]
    initial_length = tf.shape(log_probs)[1]

    rows = tf.tile(tf.reshape(tf.range(batch_size, dtype=tf.int32), [batch_size, 1]), [1, initial_length])
    cols = tf.tile(tf.reshape(tf.range(initial_length, dtype=tf.int32), [1, initial_length]), [batch_size, 1])
    indexes = tf.stack([rows, cols, initial_ids], axis=-1)
    positional_log_probs = tf.gather_nd(log_probs, indexes)
    initial_log_probs = tf.reduce_sum(positional_log_probs, axis=1)
    return initial_log_probs


def _flat_clip_last_length(tensor):
    """Reduces sequence length by 1 for flat cache.

    Args:
      tensor: Tensor to reduce of shape [batch_size*beam_size, length, ...]

    Returns:
      Reduced tensor of shape [batch_size*beam_size, length-1, ...]
    """
    shape = _shape_list(tensor)
    shape[1] -= 1
    begin = [0 for _ in shape]
    return tf.slice(tensor, begin=begin, size=shape)
