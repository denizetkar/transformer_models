...
# ==============================================================================
"""Language model using trained transformer_decoder."""
...
def generate_from_file(
        ...
        print_all_generations=True):
    """Generate lines from initial tokens in file, and save to output file if specified.

    Args:
      estimator: tf.Estimator used to generate the generations.
      subtokenizer: Subtokenizer object for encoding and decoding source and
         generated lines.
      input_file: file containing tokens to start generating
      output_file: file that stores the generations.
      print_all_generations: If true, all generations are printed to stdout.
    ...
    # in sorted list) to write generations in the original order.
    ...
    for i, prediction in enumerate(estimator.predict(input_fn)):
        generation = _trim_and_decode(prediction["outputs"], subtokenizer)
        generations.append(generation)

        if print_all_generations:
            tf.logging.info("Generating:\n\tInput: %s\n\tOutput: %s" %
                            (sorted_inputs[i], generation))

    # Write generations in the order they appeared in the original file.
...
def generate_text(estimator, subtokenizer, txt):
    ...
    generation = next(predictions)["outputs"]
    generation = _trim_and_decode(generation, subtokenizer)
    tf.logging.info("Generation of \"%s\": \"%s\"" % (txt, generation))


def main(unused_argv):
    from official.transformer_decoder import transformer_main

    tf.logging.set_verbosity(tf.logging.INFO)

    if FLAGS.text is None and FLAGS.file is None:
        tf.logging.warn("Nothing to generate. Make sure to call this script using "
                        "flags --text or --file.")
        return
    ...
    if FLAGS.text is not None:
        tf.logging.info("Generating text: %s" % FLAGS.text)
        generate_text(estimator, subtokenizer, FLAGS.text)

    if FLAGS.file is not None:
        input_file = os.path.abspath(FLAGS.file)
        tf.logging.info("Generating from file: %s" % input_file)
        ...
        generate_from_file(estimator, subtokenizer, input_file, output_file)


def define_generate_flags():
    """Define flags used for generation script."""
    # Model flags
    flags.DEFINE_string(
        name="model_dir", short_name="md", default="/tmp/transformer_model",
        help=flags_core.help_wrap(
            "Directory containing TransformerDecoder model checkpoints."))
    flags.DEFINE_enum(
    ...
    flags.DEFINE_string(
        ...
        help=flags_core.help_wrap(
            "Text from which to generate. Output will be printed to console."))
    flags.DEFINE_string(
        name="file", default=None,
        help=flags_core.help_wrap(
            "File containing text from which to generate. Generation will be printed "
            "to console and, if --file_out is provided, saved to an output file."))
    flags.DEFINE_string(
        name="file_out", default=None,
        help=flags_core.help_wrap(
            "If --file flag is specified, save generation to this file."))
