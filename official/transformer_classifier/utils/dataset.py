...
# ==============================================================================
"""Input pipeline for the transformer_classifier model to read, filter, and batch examples.
...
1. Batching scheme
   ...
   Where integers in the arrays refer to tokens in the vocab file.

   Prior to batching, elements in the dataset are grouped by length.
   Each group is then batched such that:
   ...
   Lengths are padded to the longest "inputs" sequence in the batch.
...
2. Shuffling
   ...
   `parallel_interleave`, the `sloppy` argument is used to create randomness
...
def _batch_examples(dataset, batch_size, max_length):
    ...
    Args:
      ...
      max_length: Max number of tokens in an example input sequence.
    ...
    def example_to_bucket_id(example_input, _):
...
def _read_and_batch_from_files(
        ...
        static_batch=False):
    """Create dataset where each item is a dict of "inputs" and "label".
    ...
    # Remove examples where the input length exceeds the maximum length,
    dataset = dataset.filter(lambda x, y: _filter_max_length(x, max_length))
