...
# ==============================================================================
"""Train and evaluate the TransformerClassifier model.
...
from official.transformer_classifier.model import model_params
from official.transformer_classifier.model import transformer
from official.transformer_classifier.utils import dataset
from official.transformer_classifier.utils import metrics
from official.transformer_classifier.utils import schedule
...
def model_fn(features, labels, mode, params):
    ...
    with tf.variable_scope("model"):
        ...
        if mode == tf.estimator.ModeKeys.EVAL:
            ...
            return tf.estimator.EstimatorSpec(
                mode=mode, loss=loss, predictions={"predictions": logits},
                eval_metric_ops=metrics.get_eval_metrics(logits, inputs, params))
        else:
...
def define_transformer_flags():
    ...
    # Add transformer_classifier specific flags
    flags.DEFINE_enum(
    ...
    flags.DEFINE_string(
        ...
        help=flags_core.help_wrap(
            ...
            "the vocab file."))
    flags.DEFINE_string(
        name="transfer_model_dir", short_name="tmd", default="/tmp",
        help=flags_core.help_wrap("The location of the transfer model checkpoint files."))
    flags.DEFINE_integer(
        name="num_classes", short_name="ncls", default=None,
        help=flags_core.help_wrap(
            "The Number of classes for classification."))

    flags_core.set_defaults(data_dir="/tmp/classify_en",
    ...
    @flags.multi_flags_validator(
        ["num_classes"],
        message="--num_classes must be defined.")
    def _check_num_classes(flags_dict):
        return flags_dict["num_classes"] is not None

    flags_core.require_cloud_storage(["data_dir", "model_dir", "transfer_model_dir", "export_dir"])
...
def construct_estimator(flags_obj, params, schedule_manager):
    ...
    if not params["use_tpu"]:
        ...
        if flags_obj.per_process_gpu_memory_fraction is not None:
            gpu_options = tf.GPUOptions(per_process_gpu_memory_fraction=flags_obj.per_process_gpu_memory_fraction)
...
def run_transformer(flags_obj):
    """Create tf.Estimator to train and evaluate transformer_classifier model.
    ...
    params["transfer_model_dir"] = flags_obj.transfer_model_dir
    params["num_classes"] = flags_obj.num_classes
    ...
    # Train and evaluate transformer_classifier model
